...
const GACHA_ITEMS = [
  { name: "오늘의 밸런스 게임", rarity: "COMMON", weight: 30, emoji: "⚖️", description: "조원 각자 A/B 중 하나를 골라 이유를 말해보세요." },
  ...
  { name: "인간관계 난이도 테스트", rarity: "RARE", weight: 8, emoji: "🧗", description: "나의 인간관계 난이도는 몇 점일까요? 결과를 조원과 비교해 보세요.", linkUrl: "https://test-it.co.kr/test414.html" },
...
async function main() {
  ...
  for (let t = 1; t <= TEAM_COUNT; t++) {
    ...
    for (let z = 1; z <= ZONES_PER_TEAM; z++) {
      const zone = await prisma.zone.create({ data: { name: `${t}팀 ${z}조`, order: z, teamId: team.id } });
